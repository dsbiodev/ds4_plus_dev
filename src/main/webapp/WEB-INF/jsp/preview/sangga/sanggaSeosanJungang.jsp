...
<style>
	...
	.appllationH { font-size: 24px; width: 100%; text-align: center; display: flex; justify-content: center; align-items: center; }
		
	.goin-wrap { display: flex; justify-content: center; align-items: center; height: 100%; white-space: pre-wrap; }	
	.goinH { font-size: 22px; width: 100%; text-align: center; display: flex; justify-content: center; align-items: center; }
		
		
	.sangjuNm-wrap { display: flex; justify-content: center; align-items: center; height: 100%; white-space: pre-wrap; }	
	.sangjuNmH { font-size: 22px; width: 100%; text-align: left; display: flex; justify-content: flex-start; align-items: center; }
	
	.jangji-wrap { display: flex; justify-content: center; align-items: center; height: 100%; white-space: pre-wrap; }
 	.jangjiH { font-size: 20px; width: 100%; text-align: left; display: flex; justify-content: center; align-items: center; }
	...
	function sanggaInit(_funeralNo){
	     $.pb.ajaxCallHandler('/admin/selectSanggaList.do', { order:'EXPOSURE ASC, APPELLATION ASC', funeralNo:_funeralNo, statusPlate:true }, function(data) {
	        ...
	        $.each(_list, function(i, oneData){
				...
				var dy = '';
				/* var cause = '' */
				var enterDt = '<td style="width:3%;">입실</td><td style="width:8%;"></td>';
				...
				if(oneData.APPELLATION !=null && oneData.APPELLATION !=""){
	           	...
	           	if(oneData.DM_NAME != null && oneData.DM_NAME != ""){
                 	...
                 	if(oneData.DM_GENDER!=0){
	              	...
	              	if(oneData.strSangju != null && oneData.strSangju != ""){
	              		...
	              		//console.log(tmpStr);
	              		/* sangjuNm += tmpStr.replace(/\\/gi, ') ('); */ //상주 정보 가로 타입
	              		sangjuNm += tmpStr.replace(/\\/gi, ')<br>(');	 //상주 정보 세로 타입             		
	              	}	              	
	           	...
	           	tbody.append('<tr>'+
	           			/* '<td rowspan="3" style="width:7%;"><div class="appllation-wrap"><div class="appllationH">'+f_parlor+'<br>'+f_parlor_floor+'</div></div></td>'+ */
	           			'<td rowspan="3" style="width:7%;"><div class="appllation-wrap"><div class="appllationH">'+oneData.APPELLATION+'</div></div></td>'+
		           		'<td rowspan="3" style="width:10%;"><div class="goin-wrap"><div class="goinH">'+nameAreaStr+'</div></div></td>'+
		           		'<td rowspan="3" style="width:32%;"><div class="sangjuNm-wrap"><div class="sangjuNmH">'+sangjuNm+'</div></div></td>'+enterDt+
		           	...
		           	'<td style="width:3%;"><font color="lightgray"></font></td><td style="width:3%;"><div class="ellipsis" style="width: 100%; padding: 0px;">'+""+'</div></td>'+
		           	'<td rowspan="3" style="width:17%;"><div class="bigo-wrap"><div class="bigo">'+bigo+'</div></div></td>'+		           	
	        ...
	        if(dataLen <= 5){//64px
	           //var rHeight = '64px';
	           var rHeight = ((950 / (dataLen * 4)) + 1 ) ; //18
	           
	           $('.sangga-table-body').find('td').css('font-size','17px');
	           
	           $('.sangga-table-body .sangjuNm-wrap').height((rHeight*3+26)+'px');
	           $('.sangga-table-body .goin-wrap').height((rHeight*3-30)+'px');
	           $('.sangga-table-body .bigo-wrap').height((rHeight*3+26)+'px');
	           $('.sangga-table-body .font-wrap').height((rHeight*2+20)+'px');	//HYH - font-size 관련 추가
	           $('.sangga-table-body .jangji-wrap').height((rHeight*2)+'px');	//HYH - font-size 관련 추가
	        }else{
	        	/*
	           if(dataLen > 10){
	           ...
	           */
	           ...
	           /* 
	           var rHeight = ((950 / (dataLen * 4)) - 3 ) ; //18
	           //var rHeight = 120 ; //18
	           $('.sangga-table-body').find('td').css('font-size','17px');
