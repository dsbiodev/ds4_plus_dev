...
<style>
	...
	.jangji-wrap { display: flex; justify-content: center; align-items: center; height: 100%; white-space: pre-wrap; }
 	...
 	.bigo { font-size: 23px; width: 100%; text-align: left; display: flex; justify-content: flex-start; align-items: center; }
 	.appllationH { font-size: 30px; width: 100%; text-align: center; display: flex; justify-content: center; align-items: center; }
 	
 	.goinH { font-size: 30px; width: 100%; text-align: center; display: flex; justify-content: space-around; align-items: center; }
 	.sangjuNmH { font-size: 24px; width: 100%; text-align: left; display: flex; justify-content: flex-start; align-items: center; }
 	.jangjiH { font-size: 24px; width: 100%; text-align: left; display: flex; justify-content: space-around; align-items: center; }
 	.sangjoH { font-size: 24px; width: 100%; text-align: left; display: flex; justify-content: space-around; align-items: center; }
...
<script>
	function sanggaInit(_funeralNo){
	     $.pb.ajaxCallHandler('/admin/selectSanggaList.do', { order:'EXPOSURE ASC, APPELLATION ASC', funeralNo:_funeralNo, statusPlate:true }, function(data) {
	        ...
	        thead.append('<tr><td>빈소</td><td>고인</td><td>상주정보</td><td colspan="2" style="letter-spacing:1.2px;">상례정보</td><td>장지</td><td colspan="2">정보</td><td style="letter-spacing:1px;">입실 대기</td><td>비고</td></tr>');
	        ...
	        $.each(_list, function(i, oneData){
				...
				*/
				var f_parlor="";			//빈소명
				var f_parlor_floor="";		//빈소 층 
				if(oneData.APPELLATION !=null && oneData.APPELLATION !=""){
					//var tmp1=oneData.APPELLATION.split('(');
					//var tmp2="("+tmp1[1]
					var tmp1=oneData.APPELLATION.substring(0,6);
					var tmp2=oneData.APPELLATION.substring(6);
				...
				if(oneData.DM_NAME != null && oneData.DM_NAME != ""){
	              	...
	              	/**
	              	* 고인 정보
	              	* 요청사항 : 고인명, 성별, 나이, 종교까지 있을 경우 3줄로 표현, 폰트 굵게
	              	* TODO : 폰트 굵게
	              	*/
	           	...
	           	tbody.append('<tr>'+
	           		...
	           		//'<td rowspan="3" style="width:10%;">'+nameAreaStr+'</td>'+															//고인명, 성별, 나이
	           		'<td rowspan="3" style="width:13%;"><div class="font-wrap"><div class="goinH">'+nameAreaStr+'</div></div></td>'+
	           		//'<td rowspan="3" style="width:30%;"><div class="sangjuNm-wrap"><div class="sangjuNm">'+sangjuNm+					//상주관계 
	           		'<td rowspan="3" style="width:33%;"><div class="sangjuNm-wrap"><div class="sangjuNmH">'+sangjuNm+ '</div></div></td>' //상주관계        			           		
	           		+ enterDt+																									//입실, 입관, 발인
	           		...
	           		'<td rowspan="3" style="width:10%;"><div class="jangji-wrap"><div class="jangjiH">'+jangji+'</div></div></td>'+	//장지
		           	
	           		'<td rowspan="3" style="width:9%;">'+sangjo+'</td>'+																			//상조
		           	
	           		
	           		
	           		'<td style="width:3%;">사인</td><td style="width:6%;"><div class="ellipsis" style="width: 100%; padding: 0px;">'
	           		+cause+'</div></td>'+
		           	'<td rowspan="3" style="width:6%;">'+cause+'</td>'+ 
					...
					'<td rowspan="3" style="width:9%;"><div class="jangji-wrap"><div class="jangjiH">'+jangji+'</div></div></td>'+		           	
		           	
		           	
		           	'<td style="width:3%; ">&nbsp;</td><td style="width:4%;"><div class="ellipsis" style="width: 100%; padding: 0px;">' + '&nbsp;' +'</div></td>'+
		           	'<td rowspan="3" style="width:8%;"><div class="font-wrap"><div class="sangjoH">'+sangjo+'</div></div></td>'+
		           	'<td rowspan="3" style="width:12%;"><div class="bigo-wrap"><div class="bigo">'+bigo+'</div></div></td></tr>'+
	        ...
	        if(dataLen <= 5){//64px
	           ...
	           $('.sangga-table-body .jangji-wrap').height((rHeight+20)+'px');	//HYH - font-size 관련 추가
	        ...
	        $('.sangga-table-body .sangjoH').each(function() {//입실대기 = 상조
				autoFontSize( $(this), $(this).parent('.font-wrap'));
	        });
