...
.religionLogo { display: flex; align-items: center; justify-content:center;  padding-left: 92px; padding-bottom: 7px; }
.religionLogo .normal_religion { border-top-left-radius: 25px; border-bottom-left-radius: 25px;}
.religionLogo .3_buddhism { border-top-left-radius: 0px; border-bottom-left-radius: 0px;}
</style>
<head>
<script>
	function makeUI(data, index) {
		...
		if(style[0].BOTTOM_TEXT != ''){
			...
			$('.swiper-slide.page.p'+index).css('background-image', 'url("'+imgUrl+'/dsfiles/division/img_division_one_bottom_black_'+style[0].STATUS_PLATE_STYLE_NO+'.png")');
		}else{
			// 하단 문구가 없는 경우 //
			var basicLayer = $('<div class="one-basic-layer" style="width:100%; height:100%; display:flex; flex-direction: column; ">');
			...
			basicLayer.append('</div>');
			$('.swiper-slide.page.p'+index).html(basicLayer);
			$('.swiper-slide.page.p'+index).css('background-image', 'url("'+imgUrl+'/dsfiles/division/img_division_one_top_black_'+style[0].STATUS_PLATE_STYLE_NO+'.png")');
		}
		...
		$.each(evt, function(i, evtItem){
			console.log("==== 종교 정보 ===> " + evt[index].DM_RELIGION);
			var imgPath = "";
			
			if(evtItem.DM_RELIGION == 1){		//기독교
				imgPath='<div class="religionLogo"><img class="normal_religion" src="/resources/img/ongoul/1_christian.png" alt="national christian" width="100" height="90"></div>';
			}else if(evtItem.DM_RELIGION == 2){ //천주교
				imgPath='<div class="religionLogo"><img class="normal_religion" src="/resources/img/ongoul/2_catholic.png" alt="national catholic" width="100" height="90"></div>';
			}else if(evtItem.DM_RELIGION == 3){ //불교
				imgPath='<div class="religionLogo"><img class="3_buddhism" src="/resources/img/ongoul/3_buddhism.png" alt="national buddhism" width="75" height="75"></div>';
			}else if(evtItem.DM_RELIGION == 4){ //원불교
				imgPath='<div class="religionLogo"><img class="normal_religion" src="/resources/img/ongoul/4_wonbuddhism.png" alt="national wonbuddhism" width="82" height="82"></div>';
			}else if(evtItem.DM_RELIGION == 5){ //국가유공자
				imgPath='<div class="religionLogo"><img class="normal_religion" src="/resources/img/ongoul/5_merit.png" alt="national merit" width="234" height="90" ></div>';
			}else if(evtItem.DM_RELIGION == 6){
				imgPath='<div class="religionLogo"><img class="normal_religion" src="/resources/img/ongoul/6_user.png" alt="national nothing" width="234" height="89" ></div>';
			}
			...
			$('.binso.i'+index+'.b'+i).append('<div class="binso-top"><div class="bin-top-left"><div class="bin-hosil">'+evt[index].APPELLATION+'</div></div>' + imgPath  + '<div class="bin-top-center"><div class="bin-goin">'+nameAreaStr+'</div></div><div class="bin-top-right arrow"></div></div>');
			
			if(style[0].ARROW_FLAG == 1 && evt[index].ARROW_NO != 0 && evt[index].ARROW_NO) $('.binso.i'+index+'.b'+i).find('.arrow').css('background-image', 'url("'+imgUrl+'/dsfiles/arrow/icon_arrow_'+evt[index].ARROW_NO+'.svg")').css('background-color', '#FFF');
