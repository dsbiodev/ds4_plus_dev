...
// 			basicLayer.append('<div class="title-wrap"><div class="funeral-logo"></div></img><div class="title-center" style="font-size:45px;">빈 소 안 내</div><div class="title-right">'+new Date().format('MM월 dd일 HH시 mm분')+'</div></div>');
//			basicLayer.append('<div class="title-wrap"><div class="funeral-logo" style="background-position: 0% 50%;"></div></img><div class="title-center" style="font-size:45px;">빈 소 안 내</div><div class="title-right"></div></div>');
			basicLayer.append('<div class="title-wrap"><div class="funeral-logo" style="background-position: 0% 50%;"></div></img><div class="title-center" style="font-size:45px;">빈 소 안 내</div><div class="title-right">'+new Date().format('MM월 dd일 HH시 mm분')+'</div></div>');
...
//화면 분할 분기 구분
			...
			if(funeral[0].FUNERAL_NO != 940){	//일반 화면 분기
				...
				if(evtLen == 0){
					...
					// 1분할 //
					loadDivOneOngoul(data);
				}else if (evtLen == 2) {
					// 2분할 //
					loadDivTwoOngoul(data);
				}else if (evtLen <= 4) {
					// 4분할 //
					loadDivFourOngoul(data);
				}else if (evtLen <= 6) {
					// 6분할 //
					loadDivSixOngoul(data);
				}else if (evtLen >= 7){
					// 8분할, 8개이상 시 스크롤 //
					loadDivEightOngoul(data);
				}else{
					//  //
				}
			}
	...
	//온고을
	function loadDivOneOngoul(data){
		$('.data-previews').pageConnectionHandler('/preview/jonghapMode/component/ongoul/shared1', { }, function() { initUI(data) });
	}
	function loadDivTwoOngoul(data){
		$('.data-previews').pageConnectionHandler('/preview/jonghapMode/component/ongoul/shared2', { }, function() { initUI(data) });
	}
	function loadDivFourOngoul(data){
		$('.data-previews').pageConnectionHandler('/preview/jonghapMode/component/ongoul/shared4', { }, function() { initUI(data) });
	}
	function loadDivSixOngoul(data){
		$('.data-previews').pageConnectionHandler('/preview/jonghapMode/component/ongoul/shared6', { }, function() { initUI(data) });
	}
	function loadDivEightOngoul(data){
		$('.data-previews').pageConnectionHandler('/preview/jonghapMode/component/ongoul/shared8', { }, function() { initUI(data) });
	}
