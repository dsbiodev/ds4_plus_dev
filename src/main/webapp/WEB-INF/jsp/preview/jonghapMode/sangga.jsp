...
<style>
	...
	.bigo-wrap { display: flex; justify-content: center; align-items: center; height: 100%; white-space: pre-wrap; }
 	.bigo { font-size: 20px; width: 80%; text-align: left; display: flex; justify-content: flex-start; align-items: center; }
...
<script>
	function sanggaInit(_funeralNo){
	     $.pb.ajaxCallHandler('/admin/selectSanggaList.do', { order:'EXPOSURE ASC, APPELLATION ASC', funeralNo:_funeralNo, statusPlate:true }, function(data) {
	        ...
	        thead.append('<tr><td>빈소</td><td>고인</td><td>대표상주</td><td colspan="2" style="letter-spacing:1.2px;">상례정보</td><td>장지</td><td style="letter-spacing:1.2px;">상조회사</td><td colspan="2">정보</td><td>비고</td><td>상주명</td></tr>');
	        ...
	        $.each(_list, function(i, oneData){
				...
				var carryDt = '<td>발인</td><td></td>';
	           	...
	           	tbody.append('<tr>'+
	           		'<td rowspan="3" style="width:10%;">'+oneData.APPELLATION+'</td>'+
	           		'<td rowspan="3" style="width:12%;">'+nameAreaStr+'</td>'+
	           		'<td rowspan="3" style="width:12%;">'+sjName+'</td>'+enterDt+
		           	'<td rowspan="3" style="width:10%; "><div style="width:100%; height:100%; display:flex; justify-content: center; align-items: center; white-space: pre-wrap;">'+jangji+'</div></td>'+
		           	...
		           	'<td rowspan="3" style="width:0%;"><div class="bigo-wrap"><div class="bigo">'+bigo+'</div></div></td>'+		           	
