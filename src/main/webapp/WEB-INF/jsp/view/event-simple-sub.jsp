		...
		var crtLeftMenu = function(_flag) {
			...
			if(_clickFlag == 1) {
				...
				$('.event-info-top > .right-wrap').append('<button type="button" class="btn long cal">장례비용내역서 인쇄</button>');
				 $('.event-info-top > .right-wrap').append('<button type="button" class="btn rcp">영수증 출력</button>');
			}
...
// 				window.open('/290903/'+_param.pk, "printName", "");
			...
			// 22.1.6 정산 영수증 출력
			$('.event-info-top > .right-wrap > .btn.rcp').off().on('click', function(){
				window.open('/290907/'+_param.pk, "printName", "width=1400, height=800, scrolbars=yes");
			});
		...
		$('.event-info-top > .left-wrap > .item').on('click', function() {
			...
			if($(location)[0].pathname.split('/')[2]) {
				...
				var stateObj = { link:'/2901020'+_thisIdx+(_param.pk ? '/'+_param.pk:'') };
