...
<script>
	$(function() {
	    ...
	    if ( varUA.indexOf('android') > -1) {
			////android 
	    	$('.previews-wrap').css('height',"100%");
			$('.main-contents-wrap > .contents-body-wrap').css( 'width' , '812px');
			$('.previews-wrap .screen-previews-box').css( 'width' , '812px');
			$('.previews-wrap .screen-previews-box').css( 'height' , 'auto');
			$('.previews-wrap').css( 'height' , '100%');
			$('.main-contents-wrap > .contents-body-wrap').css( 'zoom' , '0.5');
			$('.previews-wrap').css( 'overflow-x' , 'scroll');
	    	...
	    	$('.previews-wrap').css('height',"100%");
			$('.main-contents-wrap > .contents-body-wrap').css( 'width' , '812px');
			$('.contents-title-wrap').css( 'width' , '812px');
			$('.previews-wrap .screen-previews-box').css( 'width' , '812px');
			$('.previews-wrap .screen-previews-box').css( 'height' , 'auto');
			$('.previews-wrap').css( 'height' , '100%');
			$('.previews-wrap').css( 'overflow-x' , 'scroll');
				...
				$.each(tableData.list, function(idx){
					...
					$.each(tableData.event, function(){
						if(_this.RASPBERRY_CONNECTION_NO == this.RASPBERRY_CONNECTION_NO){
							...
							if(this.DM_AGE=='' && selGender==""){		//나이와 성별이 없을때
								appendText="";
							}else if(this.DM_AGE !='' && selGender == ""){	//나이는 있고 성별은 없을때
								appendText=" (" + this.DM_AGE + "세)";
							}else if(this.DM_AGE !='' && selGender != ""){	//나이와 성별이 있을때
								appendText=" (" + selGender + " ,"+this.DM_AGE+"세)";
							}else if(this.DM_AGE =='' && selGender !=""){	//나이는 없고 성별은 있을때
							...
							if(_this.SCREEN_MODE == 1){								
								...
								_div.find('.previews .name').html("故 "+this.DM_NAME+" "+isNull(this.DM_POSITION, '')+appendText);
								
							}	
							else{
								/* _div.find('.previews .name').html("<div>故 "+this.DM_NAME+" "+isNull("<br>"+this.DM_POSITION, '')+"</div><div>("+(this.DM_GENDER == 1 ? '남' : '여')+(this.DM_AGE ? ",  "+this.DM_AGE+"세" : "")+")</div>"); */
								_div.find('.previews .name').html("<div>故 "+this.DM_NAME+" "+isNull("<br>"+this.DM_POSITION, '')+"</div><div>"+appendText+"</div>");
							}
							// ./HYH 성별 추가
					...
					_div.on('click', function(){
						$('.pb-right-popup-wrap').openLayerPopup({}, function(_thisLayer) {
							...
							if(_info){
								...
								if(_info.DM_AGE=='' && selGender==""){		//나이와 성별이 없을때
									appendText="";
								}else if(_info.DM_AGE !='' && selGender == ""){	//나이는 있고 성별은 없을때
									appendText=" (" + _info.DM_AGE + "세)";
								}else if(_info.DM_AGE !='' && selGender != ""){	//나이와 성별이 있을때
									appendText=" (" + selGender + " ,"+_info.DM_AGE+"세)";
								}else if(_info.DM_AGE =='' && selGender !=""){	//나이는 없고 성별은 있을때
									appendText= " ("+ selGender+")";
								}
								
								_thisLayer.find('.name').text("故 "+_info.DM_NAME+" "+isNull(_info.DM_POSITION, '')
										+ appendText);								
								// ./HYH 성별 추가
								...
								_thisLayer.find('.photo').css('background-image', 'url("")');
...
<style>
/* 	html { transform: initial; position: fixed; top: initial; left: initial; width:100%; height:100%; } */
</style>
<form id="dataForm10">
...
<div class="contents-body-wrap" id="b-body">

	<div class="previews-wrap" id="b-warp"></div>
</div>