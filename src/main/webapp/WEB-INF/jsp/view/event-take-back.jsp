...
<script>
	$(function() {
		...
		$.pb.ajaxCallHandler('/adminSec/selectCMName.do', {	eventNo : _param.pk }, function(data) {
			$('input[name=takeBackName]').val(data.cmName.CM_NAME);
			...
			$.pb.ajaxCallHandler('/adminSec/selectOrderTotalList.do', {	eventNo : _param.pk }, function(data) {
				...
				$.each(data.list, function(idx) {
					...
					_tr.find('input[name=cnt]').plusMinusBox(
		 				function() {
		 					_tr.find('.total-price').text($.pb.targetMoney(_this.VAT_PRICE*((_this.ORDER_CNT ? _this.ORDER_CNT : 0)*1 - _tr.find('input[name=cnt]').val()*1))+"원")
		 					calTotalPrice();
		 					if(_tr.find('input[name=cnt]').val() == 0) _tr.removeClass('ac');
		 				},function() {
		 					if(_tr.find('input[name=cnt]').val() > _this.ORDER_CNT) _tr.find('input[name=cnt]').val(_this.ORDER_CNT)
		 					_tr.find('.total-price').text($.pb.targetMoney(_this.VAT_PRICE*((_this.ORDER_CNT ? _this.ORDER_CNT : 0)*1 - _tr.find('input[name=cnt]').val()*1))+"원")
		 					calTotalPrice();
					...
					$.each(data.list, function(idx) {
						...
						_div.on('click', function(){
							...
							$.pb.ajaxCallHandler('/adminSec/selectSetItemList.do', {setNo : _div.data('SET_NO')}, function(data) {
								var _list = [];
								$.each(data.list, function(idx) {
									var _tmp = addSetItem(this, 'set');
									if(_tmp) _list.push(_tmp); 
								});
								if(_list.length > 0) return alert("["+_list.join() +"]" + " 주문된 물품이 없습니다.")
							});
						});
					});
				});
		...
		var addSetItem = function(_data, flag) {
			var _tr = $('<tr class="order">');
			_tr.data(_data);
			...
			_tr.find('input[name=cnt]').plusMinusBox(
				function(){
 					_tr.find('.total-price').text($.pb.targetMoney(_data.PRICE*(_tr.find('.order-cnt').text()*1 - _tr.find('input[name=cnt]').val()*1))+"원");
 					calTotalPrice();
 					if(_tr.find('input[name=cnt]').val() == 0) _tr.removeClass('ac');
 				},function(){ 
 					if(_tr.find('input[name=cnt]').val() > _this.ORDER_CNT) _tr.find('input[name=cnt]').val(_tr.find('.order-cnt').text()*1)
 					_tr.find('.total-price').text($.pb.targetMoney(_data.PRICE*(_tr.find('.order-cnt').text()*1 - _tr.find('input[name=cnt]').val()*1))+"원");
 					calTotalPrice();
 					if(_tr.find('input[name=cnt]').val() > 0) _tr.addClass('ac');
 				},function(){
			...
			if($('.order-box tbody tr').length > 0) {
				$('.order-box tbody tr').each(function() {
					if($(this).data('ITEM_NO') == _data.ITEM_NO) {
						_tmp = 0;
						if($(this).find('.order-cnt').text()*1 > _data.CNT*1) {
		 					$(this).find('input[name=cnt]').val(parseInt(_data.CNT ? _data.CNT : 1));
						}else {
							$(this).find('input[name=cnt]').val(parseInt(1));
						}
						$(this).find('.total-price').text($.pb.targetMoney(_data.PRICE * ($(this).find('.order-cnt').text()*1 - $(this).find('input[name=cnt]').val()*1))+"원")
				...
				if(!flag) {
					if(_tmp != 0) return alert("주문된 물품이 없습니다.");
				}else {
					if(_tmp != 0) {
		...
		$('.btn-info .total-order-list').on('click', function() {
			...
			$.pb.ajaxCallHandler('/adminSec/selectEventFuneralInfoList.do', { eventNo : _param.pk, flag:'takeback' }, function(data) {
	 		   ...
	 		   $('.order-box tbody tr').each(function() {
					...
					_str += FnOrderTrans($(this).data(), 'tb');
		...
		$('.btn-info .re-print').on('click', function() {
			...
			$.pb.ajaxCallHandler('/adminSec/selectEventOrderRePrintList.do', { eventNo : _param.pk, flag:'takeback', funeralNo : '${sessionScope.loginProcess.FUNERAL_NO}' }, function(data) {
				$('object').remove();
				var _payObj = $('<object>');
				_payObj.attr('classid', 'CLSID:505A0E24-D6BB-4B0B-A46D-92F3D558AF21');
				_payObj.attr('codebase', '/resources/js/NicePrintV100.cab#version=1,0,0,1');
		    	...
		    	for(var i = 0; i < (39 - _totalLen); i++) { _totalStr += " "; }
		    	_totalStr += $.pb.targetMoney("-"+_totalPrice);
	 			transdata += "합계금액"+_totalStr+"\n";
	 		    transdata += "------------------------------------------------\n";
	 			transdata += "거 래 일 : "+new Date().format('yyyy년 MM월 dd일');
				...
				$.each(data.partner, function(idx) {
					...
					$.each(data.list, function(idx) {
		 		    	_strP += FnOrderPrint(this, _this.PARTNER_FLAG, 'tb');
			    	...
			    	for(var i = 0; i < (32 - _totalLenP); i++) { _totalStrP += " "; }
			...
			$.pb.ajaxCallHandler('/adminSec/selectEventFuneralInfoList.do', { eventNo : _param.pk, funeralNo : '${sessionScope.loginProcess.FUNERAL_NO}' }, function(data) {
				...
				var printTxt = function(txt01) {
			    	...
			    	_totalStr += $.pb.targetMoney(_tPrice);
		 			transdata += "합계금액"+_totalStr+"\n";
		 		    transdata += "------------------------------------------------ \n";
		 			transdata += "반품자 : "+$('input[name=takeBackName]').val()+ "\n"
		 			transdata += "------------------------------------------------ \n";
		 			transdata += "\n서명\n\n\n"
		 			transdata += "------------------------------------------------ \n";
		 			transdata += "거 래 일 : "+new Date().format('yyyy년 MM월 dd일');
				...
				printTxt("반 품 고 객 용", 1);
				printTxt("반 품 가 맹 용", 1);
				...
				$.each(data.partner, function(idx) {
					...
					$('.order-box tbody tr').each(function() {
						if($(this).find('input[name=cnt]').val() > 0 && _this.PARTNER_FLAG == $(this).data('PARTNER_FLAG')) {
							$(this).data("CNT", $(this).find('input[name=cnt]').val())
							_strP += FnOrderPrint($(this).data(), _this.PARTNER_FLAG, 'tb');
					...
					var printTxtP = function(txt01) {
				    	...
				    	printdata += "합계금액"+_totalStrP+"\n";
				    	printdata += "------------------------------------------\n";
				    	printdata += "반품자 : "+$('input[name=takeBackName]').val()+ "\n"
				    	printdata += "------------------------------------------\n";
				    	printdata += "\n서명\n\n\n"
				    	printdata += "------------------------------------------\n";
			 		    printdata += "거 래 일 : "+new Date().format('yyyy년 MM월 dd일');
...
// 					printTxtP("고 객 용", _this.PARTNER_FLAG);
					printTxtP("반 품 가 맹 용", _this.PARTNER_FLAG);
				});
...
<style>
	.event-info-box .event-info-btn-box .event-info-btn-wrap .order-info tbody td.title { background: #eec8c8; }
	.event-info-box .event-info-btn-box .event-info-btn-wrap .register.ac { background-image: linear-gradient(to bottom, #efcfcf, #de3a3a); }
	.event-info-box .mid-wrap .mid-box .company.ac { background:#eec8c8; }
	.event-info-box .list-wrap .classification-box div.ac { background:#eec8c8; }
	.event-info-box .list-wrap .order-box tbody tr.order.ac { background: #eec8c8; }
</style>
	...
	<div class="event-info-btn-box">
		<div class="event-info-btn-wrap">
			<table class="order-info">
				<tbody>
					<tr>
						<td class="title">반품자</td>
						<td><input type="text" class="form-text" name="takeBackName" placeholder="직접입력"/></td>
					</tr>
					<tr>
						<td class="title">반품금액</td>
			...
			<div class="btn-info">
				<div>
					<button type="button" class="item-register" disabled="disabled">임의상품등록</button>
					<button type="button" class="total-order-list">전체 주문내역 출력</button>
				</div>
				<div>
					<button type="button" class="reset">현재 반품 취소</button>
