	...
	$(function() {
		...
		var crtLeftMenu = function(_flag) {
			...
			$('.event-info-top > .right-wrap').find('.btn.cancel').off().on('click', function() { 
				if(confirm("취소하시겠습니까?")) $(location).attr('href', '/991001'); 
			});
...
// 				window.open('/290903/'+_param.pk, "printName", "");
		...
		$('.event-info-top > .left-wrap > .item').on('click', function() {
			...
			if(_pk[1]) {
				...
				var stateObj = { link:'/9910010'+_thisIdx+(_param.pk ? '/'+_param.pk:'') };
...
<div class="event-info-top">
	<div class="left-wrap">
		<button type="button" class="item info" value="info"><span class="title">행사정보</span></button>
