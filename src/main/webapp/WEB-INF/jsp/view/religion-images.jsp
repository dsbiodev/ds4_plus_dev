...
<script>
	$(function() {
		...
		for(i=1; i<=14; i++){
			var _div = $('<div>');
			_div.append('<input type="radio" name="religion"><img src="'+imgUrl+'/dsfiles/religion/christian/christian_'+i+'.png" value='+i+'>');
			$('.religion-wrap').find('.rel-box.christian').append(_div);
		}
		for(i=1; i<=19; i++){
			var _div = $('<div>');
			_div.append('<input type="radio" name="religion"><img src="'+imgUrl+'/dsfiles/religion/catholic/catholic_'+i+'.png" value='+i+'>');
			$('.religion-wrap').find('.rel-box.catholic').append(_div);
		}
		for(i=1; i<=26; i++){
			var _div = $('<div>');
			_div.append('<input type="radio" name="religion"><img src="'+imgUrl+'/dsfiles/religion/buddhism/buddhism_'+i+'.png" value='+i+'>');
			$('.religion-wrap').find('.rel-box.buddhism').append(_div);
		}
		
		for(i=1; i<=2; i++){
			var _div = $('<div>');
			_div.append('<input type="radio" name="religion"><img src="'+imgUrl+'/dsfiles/religion/wonbuddhism/wonbuddhism_'+i+'.png" value='+i+'>');
			$('.religion-wrap').find('.rel-box.wonbuddhism').append(_div);
		}
		for(i=1; i<=10; i++){
			var _div = $('<div>');
			_div.append('<input type="radio" name="religion"><img src="'+imgUrl+'/dsfiles/religion/merit/merit_'+i+'.png" value='+i+'>');
			$('.religion-wrap').find('.rel-box.merit').append(_div);
		}
		for(i=1; i<=11; i++){
			var _div = $('<div>');
			_div.append('<input type="radio" name="religion"><img src="'+imgUrl+'/dsfiles/religion/etc/etc_'+i+'.png" value='+i+'>');
		...
		$.pb.ajaxCallHandler('/common/selectCommonCode.do', { target:'RELIGION' }, function(codeResult) {
			...
			$.pb.ajaxCallHandler('/admin/selectStatusPlateBg.do', { funeralNo:'${sessionScope.loginProcess.FUNERAL_NO}' }, function(bgRst) {
				$.each(bgRst, function() {
					var _urlFile = this.FILE;
					var _rTarget = $('.screen-item[data-religion-no="'+this.RELIGION_NO+'"]');
					if(_rTarget.length) {
						if(_urlFile != null && _urlFile.indexOf("/dsfiles") > -1) {
							_urlFile = _urlFile.substring(_urlFile.indexOf("/dsfiles"),_urlFile.length);
							this.FILE = imgUrl + _urlFile;
						}
