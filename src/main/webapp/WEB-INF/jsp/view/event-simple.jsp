...
<script>
	$(function() {
		...
		console.log("event_simple ::: ", _param);
...
// 		searchObj.orderNo = 1;
		...
		var createTable_list = function(_searchObj, _flag) {
			$.pb.ajaxCallHandler('/admin/selectEventFamily.do', { funeralNo:'${sessionScope.loginProcess.FUNERAL_NO}' }, function(data) { 
				...
				$.pb.ajaxCallHandler('/admin/'+_url+'.do', _searchObj, function(tableData) {
					...
					console.log("event-simple.jsp(리스트)=> : tableData", tableData);
					...
					$.each((_flag == 'now' ? tableData:tableData.list), function(idx) {
						...
						if(this.EVENT_NO){
							if(this.IPGWAN_YN==1){
								var _entSDt = "입관 : "+ersd.format('MM월 dd일 HH:mm');
							}else{
								var _entSDt = "입관 : 미정 ";
							}
						}else{
						...
						if(this.EVENT_NO){
							if(this.CARRYING_YN==1){
								var _carDt = "발인 : "+cd.format('MM월 dd일 HH:mm');
							}else{
								var _carDt = "발인 : 미정 ";
							}
						}else{
						...
						_tr.on('click', function() {
	 						...
	 						if(_eventNo) {
	 							var stateObj = { 
	 								link:'/29010201/'+_eventNo, 
	 								page:'/manager/29010200'
	 							};
	 							var filter = "win16|win32|win64|mac|macintel";
								if (filter.indexOf(navigator.platform.toLowerCase()) < 0) {
		 							stateObj = { 
			 								link:'/29010101/'+_eventNo, 
	 							...
	 							$('.main-contents-wrap').pageConnectionHandler(stateObj.page, { pk:_eventNo });
	 						} else {
	 							var stateObj = { 
										link:'/29010201/', 
										page:'/manager/29010200'
								...
								history.pushState(stateObj, '', stateObj.link);
								$('.main-contents-wrap').pageConnectionHandler(stateObj.page, { pk:_eventNo, eventBinsoNo:_tr.data('RASPBERRY_CONNECTION_NO') });
	 						}
						});
		...
		var createTable_div = function(_searchObj, _flag) {
			$.pb.ajaxCallHandler('/admin/selectEventFamily.do', { funeralNo:'${sessionScope.loginProcess.FUNERAL_NO}' }, function(data) { 
				...
				var _url = (_flag == 'now' ? 'selectAllEventList':'selectEventList');
				$.pb.ajaxCallHandler('/admin/'+_url+'.do', _searchObj, function(tableData) {
					pkCode=tableData.funeralNo;
					console.log("pkCode : ", pkCode);
					console.log("** tableData.funeralNo ** : ", tableData.funeralNo);
					
					var _listWrap = $('.contents-body-wrap.'+_flag+' > .event-body-wrap');
					_listWrap.html('');
	 				
					
					console.log("event-simple.jsp(분할)=> : tableData", tableData);
					...
					$.each((_flag == 'now' ? tableData:tableData.list), function(idx) {
						...
						if(this.BINSO_RASPBERRY_CONNECTION_NO){
							
							_wrap.find('.dead-man-photo').append('<div class="yj"></div>');
						}
						/* var _gender = this.DM_GENDER == 1 ? "남" : "여";
					    ...
					    if(this.EVENT_NO){
					    	if(this.IPGWAN_YN == 1){					    		
					    		_wrap.append('<div class="dt">'+("입관 : " + isNull(ersdate.format('MM월 dd일 HH:mm')))+'</div>');	
					    	}else{
					    		_wrap.append('<div class="dt">'+("입관 : 미정")+'</div>');
					    	}					    	
					    }
					    ...
					    if(this.EVENT_NO){
					    	if(this.CARRYING_YN == 1){					    		
					    		_wrap.append('<div class="dt">'+("발인 : " + isNull(cdate.format('MM월 dd일 HH:mm')))+'</div>');	
					    	}else{
					    		_wrap.append('<div class="dt">'+("발인 : 미정")+'</div>');
					    	}					    	
					    }
					    /* ./HYH - 입관 발인 시간 표출 */
		...
		$('.search-right-wrap > .btn-register').on('click', function() {
			//var stateObj = { link:'/29010201', page:'/manager/29010200' };
			var stateObj = { link:'/29010401', page:'/manager/29010400' }; //hyh - 예약 페이지로 이동
		...
		//Raspberry PI Config
		$('.btn.print06').off().on('click', function(){
			//window.open('/290906/'+ _param.pk, "printName", "width=1000, height=800, scrolbars=yes");
			var popupX = (window.screen.width / 2) - (200 / 2);
			var popupY= (window.screen.height / 2) - (300 / 2);
			
			window.open('/290906/'+ searchObj.funeralNo, "printName", "width=1000, height=1000, scrolbars=yes, left='+ popupX + ', top='+ popupY");
		});
...
<div class="search-box-wrap">
	<div class="search-left-wrap">
		<!-- <div class="search-title">간편 분향실 관리 &nbsp;&nbsp;<a href="javascript:openWindowPop();"><i class="bi bi-gear"></i></a></div> -->
		<div class="search-title">간편 분향실 관리<!-- <button class="btn print06"><i class="bi bi-gear"></i></button> --></div>
	</div>
