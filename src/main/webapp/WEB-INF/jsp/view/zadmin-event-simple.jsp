...
// 		searchObj.orderNo = 1;
				...
				$.pb.ajaxCallHandler('/admin/'+_url+'.do', _searchObj, function(tableData) {
					...
					$.each((_flag == 'now' ? tableData:tableData.list), function(idx) {
						...
						_tr.on('click', function() {
	 						...
	 						if(_eventNo) {
	 							var stateObj = { 
	 								link:'/99100201/'+$('.select-box-wrap .table-search').data('funeralNo')+"&"+_eventNo, 
	 								page:'/admin/99100200'
	 							};
	 							...
	 							var stateObj = { 
	 								link:'/99100201/'+$('.select-box-wrap .table-search').data('funeralNo')+'&',  
	 								page:'/admin/99100200'
								};
								history.pushState(stateObj, '', stateObj.link);
		...
		$('.search-right-wrap > .btn-register').on('click', function() {
			if(!$('.select-box-wrap .table-search').data('funeralNo')) return alert("장례식장을 먼저 선택해 주세요.");	
			var stateObj = { link:'/99100201/'+$('.select-box-wrap .table-search').data('funeralNo')+'&', page:'/admin/99100200' };
