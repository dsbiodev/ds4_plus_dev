			...
			$.pb.ajaxCallHandler('/admin/selectFuneralMusic.do', { funeralNo : searchObj.funeralNo }, function(tableData) {
				...
				$.pb.ajaxCallHandler('/admin/selectRaspForMusic.do', { funeralNo : searchObj.funeralNo }, function(raspData) {
					...
					$.each(raspData.list, function(idx) {
						...
						var raspNo = this.RASPBERRY_CONNECTION_NO;
						...
						_tr.find('.form-select').data(this);
						...
						$('#select'+idx+'').on('change', function() {
							if(confirm('음악을 설정/변경하시겠습니까?')) {
								$.pb.ajaxCallHandler('/admin/updateRaspForMusic.do', { musicNo: $(this).val(),raspberryId: raspId, raspberryConnectionNo: raspNo, classification: $(this).data('CLASSIFICATION') }, function(result) {
									if(result) {
										closeLayerPopup();
										createTable(searchObj);
										alert('음악이 설정되었습니다.');
...
<style>
	...
	table { width:100%; text-align:center; background-color:#ffffff; }
	table > tbody > tr {  }
	table > tbody > tr.empty { height:41px; }
	table > tbody > tr.alt { background:#FAFAFA; }
	table > tbody > tr.ac { background:#98C7FF; }
	table > tbody > tr.already { background:#A8A8A8; }
	table > tbody > tr:hover { background:#98C7FF; cursor:pointer; }
	table > tbody > tr.already:hover { background:#A8A8A8; cursor:default; }
	table th { height:30px; border:1px solid #707070; background:linear-gradient(to bottom, #FFFFFF, #EBEDEE); color:#333333; font-size:16px; font-weight:500; letter-spacing:0.8px; text-align:center; cursor:pointer; }
	table td { text-align:left; padding:12px 20px; border:1px solid #707070; color:#333333; font-size:16px; font-weight:500; letter-spacing:0.8px; }
	table td.c { text-align:center; }
	table td.r { text-align:right; }
	table td.l { text-align:left; }
	table .cancellation { text-decoration:underline; }
	table .cancellation:hover { color:#E10000; }
	table > tbody .table-text { width:100%; height:56px; border:1px solid #707070; border-radius:2px; background-color:#F6F6F6; color:#333333; font-size:18px; }
	...
	<div class="pb-right-popup-wrap">
		...
		<div class="pb-popup-body">
			...
			<div class="pb-music-table">
				<table class="t-f-list" style="width:100%;">
					<thead>
						<tr>
