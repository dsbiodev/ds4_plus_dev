...
<script>
	$(function() {
		...
		var fFlag=false; //Add-화환 코드 유무 Flag
...
// 		$('.pb-right-popup-wrap input[name=id]').on('keyup', function(){ $(this).val($(this).val().replace(/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/gi, '')); });
		...
		var menuTreeControll = function(_tr) {
			var _calculate = new Array(290200, 290201, 290202, 290203, 290204, 290300, 290301, 290302, 290303);
...
// 								_thisLayer.find('input[name=id]').attr('disabled', true);
								...
								menuTreeControll(_tr); //3
								if(_tr.data('USER_MENU_NO')) {
									$.each(_tr.data('USER_MENU_NO').split(','), function(idx, _value) {
										if(_value == '290703') fFlag=true;	//Add - 기존에 화환코드가 있으면 flag=true
										_thisLayer.find('.menu-tree > .item[data-menu-no="'+_value+'"]').find('.check-box').trigger('click');
									});
								}
...
// 				_thisLayer.find('input[name=id]').attr('disabled', false);
				...
				var settingData = {};
				if(_param.lv == 20) {
					settingData.FUNERAL_NO = _param.funeralNo;
		...
		$('.pb-right-popup-wrap .top-button.register').on('click', function() {
			...
			if($('.pb-warning:visible').length) return alert('이미 사용중인 아이디입니다.');
				...
				if(_param.lv == 29 || _param.lv == 20) _formData.append('lv', _param.lv);
				_formData.append('menuNoList', _menuNo);
				
				//Add - 기존에 화환코드 존재 여부에 따라서 Insert를 실행
				var ml=_menuNo.length;
				if(fFlag == false){					
					for(i=0; i < ml; i++){
						if(_menuNo[i] == '290703'){							
							var thisFuneralNo = document.getElementsByName("funeralNo")[0].value;							
							/*
							var flowerDataObj={
									FUNERAL_NO : thisFuneralNo,
									USER_NO : _btnValue,									
							}
							*/							
							$.pb.ajaxCallHandler('/adminSec/insertFlowerFuneralCreateDt.do', {FUNERAL_NO : thisFuneralNo, USER_NO : _btnValue}, function(){
