	...
	$(function() {
		...
		var crtLeftMenu = function(_flag) {
			...
			$('.event-info-top > .right-wrap').find('.btn.cancel').off().on('click', function() { 
				if(confirm("취소하시겠습니까?")) $(location).attr('href', '/290101'); 
			});
			

			//행사 삭제
			$('.event-info-top > .right-wrap > .btn.delete').off().on('click', function(){
				if(confirm('삭제된 데이터는 복구할 수 없으며 답례문자를 발송할 수 없습니다. 삭제하시겠습니까? ')) {
	 				var _rpiBinsoList = '';
...
// 				window.open('/290902/'+_param.pk, "printName", "");
		...
		$('.event-info-top > .left-wrap > .item').on('click', function() {
			...
			if($(location)[0].pathname.split('/')[2]) {
				...
				var stateObj = { link:'/2901010'+_thisIdx+(_param.pk ? '/'+_param.pk:'') };
...
<div class="event-info-top">
	<div class="left-wrap">
		<button type="button" class="item info" value="info"><span class="title">행사</span></button>
...
<!-- 		<button type="button" class="item order-list" value="order-list"><span class="title">주문내역</span></button> -->
