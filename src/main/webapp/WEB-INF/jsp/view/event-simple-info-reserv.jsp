...
//       	$('#endDate').datepicker('setDate', '-1D'); //(-1D:하루전, -1M:한달전, -1Y:일년전), (+1D:하루후, -1M:한달후, -1Y:일년후)
		...
		//주차안내 클릭 / LSH - 수정 추가 2023.02.07 (진주센텀)
		if(strFuneralNo == '1301') {
			var parkingGuideText = "장례식장 지면주차장에는 장소가 협소하므로, 병원입구쪽(큰길가)에 타워 주차장에 주차하시면 되겠습니다.\n총 120여대 주차가 가능하며 조문객 1시간 무료주차입니다.";
			$('.body-text-area.storeInfo').text(parkingGuideText);
		}
		
		
		// 장례식장이 광혜병원 장례식장일 경우
		// 사망원인을 안치로 텍스트 수정하고
		// 상가현황에서 "안치" 타이틀과 값을 출력함.
		// 단, 염습대장에는 사망원인에 대한 값에 안치 번호가 입력되므로
		// 엽습대장의 사망원인에 대한 값은 무시해야 함.
		if(strFuneralNo != '1315') {
			var originalData=['자연사', '병사', '변사'];			
			var selectValue;
			
			selectValue=originalData;
			
			$("#deadCauseLabel").empty();
			$("#deadCause1").empty();
			
			$("#deadCauseLabel").append("사망원인");
			 
			for(var i=0; i<selectValue.length; i++){
				$("#deadCause1").append('<option value='+selectValue[i]+ '>' + selectValue[i]+'</option>');
			}
			$("#deadCause1").append('<option value="direct">' + "직접입력" +'</option>');
			
		}else{			
			var changeData=['1번', '2번', '3번', '4번', '5번', '6번'];
			var selectValue;
			
			selectValue=changeData;			
			
			$("#deadCauseLabel").empty();
			$("#deadCause1").empty();			
			
			$("#deadCauseLabel").append("안치");
			for(var i=0; i<selectValue.length; i++){
				$("#deadCause1").append('<option value='+selectValue[i]+ '>' + selectValue[i]+'</option>');
			}
			$("#deadCause1").append('<option value="direct">' + "직접입력" +'</option>');
...
// 				} else {
					...
					if(_id.indexOf('entranceRoomDtWrap') != -1) {
						...
						//퇴실일/퇴실시간 = 발인일시+1시간 / LSH - 수정 추가 2023.02.07 (부산 시민장례식장)
						if(strFuneralNo == '622') {
							var _Date = new Date(crtDt($('.carryingDtWrap')));
							var _chgDate = $.pb.createDateObj(new Date(_Date));
							_chgDate = $.pb.createDateObj(new Date(_chgDate.year, _chgDate.month-1, _chgDate.day, _chgDate.hour+1, _chgDate.min));
							crtTimeZone($('.leavingRoom'), _chgDate);							
						}
					}
// 				}
			});
		...
		/* original sourde - 발인에 대한 라디오 버튼 생성 및 버튼 처리
		...
		if(_param.pk) {
			$.pb.ajaxCallHandler('/admin/selectEventDetail.do', { eventNo:_param.pk, order:'ORDER_NO ASC' }, function(data) {
				...
				//console.log("selectEventDetail.do : ", data);
				// 수정전 빈소 배열
...
// 				$('input[nmame=eventAliveFlag]').val(_eventInfo.EVENT_ALIVE_FLAG);
			...
			//퇴실일/퇴실시간 = 발인일시+1시간 / LSH - 수정 추가 2023.02.07 (부산 시민장례식장)
			if(strFuneralNo == '622') {
				var _d4 = $.pb.createDateObj(new Date(_d.year, _d.month-1, _d.day+2, _d1.hour+1));
				crtTimeZone($('.leavingRoom'), _d4);
			}
		...
		var _familyArr = new Array('상 주', '배우자', '부 군', '미망인', '아 들', '딸', '며느리', '사 위', '자', '녀', '자 부', '손', '외 손', '손 자', '손 녀', '외손자', '외손녀', '夫 ', '妻', '子', '子婦 ', '女', '壻', '孫', '孫子', '孫女', '孫婦', '孫壻', '外孫子', '外孫女');
		...
		$(".event-simple-info-wrap .btn-save").on('click', function(event) {
			...
			if(!necessaryChecked($('#eventSubForm'))) {
				...
				$.pb.ajaxCallHandler('/admin/selectEventList.do', _binsoOlpObj, function(binsoOverlap) {
					//console.log("binsoOverlap : ", binsoOverlap);
...
// 						else if(entranceRoomOverlap.list.length) return alert('선택하신 시간에 사용중인 입관실이 있습니다.');
							...
							// 배차 등록 수정(2023.03.10)
							$.each($('.box-body.car-body .car-wrap'), function() { //.car-wrap 수정
								var _ = $(this);
			...
			<div class="profile-info-wrap" id="cmDiv">
				...
				<div class="profile-info-wrap" id = cmNote>
					<div class="row-box" style="width:calc(100% - 9px);">
						...
						<textarea class="body-text-area storeInfo" name="storeInfo" style="height:176px;" placeholder="예시글) 해당 장례식장은 코로나 확산방지를 위해 방역 수칙을 준수하고 있으며, 안전하게 방문하여 조문이 가능합니다."></textarea>
					</div>
				</div>
				...
				<div class="profile-info-wrap etc">
					...
					<!-- 사망원인 / 안치 -->
					<div class="row-box">
						<label id="deadCauseLabel" class="info-label"></label>
						<select id="deadCause1" class="body-select deadCause" name="deadCause">
