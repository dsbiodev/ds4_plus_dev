...
<script>
	$(function() {
		...
		searchObj.order = 'MANAGER_NO DESC';
		...
		var theadObj = {
			...
			theadRow:[['USER_NAME', '담당자명'], ['CNT', '진행']]
		};
		...
		var createTable = function(_searchObj) {
			$.pb.ajaxCallHandler('/adminSec/selectStatisticsUserList.do', _searchObj, function(tableData) {
				_table.find('tbody').html('');
				
				$.each(tableData.list, function(idx) {
					var _tr = $('<tr>');
					_tr.data(this);
					_tr.append('<td class="c">'+this.USER_NAME+'</td>');
		...
		$('.btn-excel').on('click', function(){
			fnExcelReport('statisticsUser', '담당자별 통계');
		});
	});
</script>
<div class="contents-title-wrap">
	<div class="title">담당자별 통계</div>
</div>
<div class="search-box-wrap">
	<div class="search-left-wrap">
		<div class="search-title">담당자별 행사 건수</div>
	</div>
...
<div class="contents-body-wrap">
	<table class="pb-table list" id="statisticsUser">
		<colgroup></colgroup>
		<thead></thead>
		<tbody></tbody>
	</table>
