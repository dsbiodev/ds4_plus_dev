...
<script>
	$(function() {
		...
		$('input[type=radio][name=stockFlag]').pbRadiobox({ addText:['차량으로 입고', '차량에서 출고'], fontSize:'16px', matchParent:true });
		...
		var createSubTable = function(){
			$.pb.ajaxCallHandler('/adminSec/selectUserStockTotalStatusList.do', {manager : $('.search-user').val()}, function(tableData) {
				var _colspan = 1;
				var _idx = 0;
				var _val = "";
		...
		var popup_list = function(){
			...
			$.pb.ajaxCallHandler('/adminSec/selectStockCompanyList.do', {order : 'CLASSIFICATION_NO, ITEM_NAME'}, function(tableData) {
				...
				$('.search-user').append('<option value="">선택</option>');	
				$.each(tableData.userList, function(idx) {
					$('.userList').append('<option value="'+this.USER_NO+'">'+this.NAME+'</option>');
					$('.search-user').append('<option value="'+this.USER_NO+'">'+this.NAME+'</option>');
				});
		...
		var theadObj = {
			table: _table,
			colGroup:new Array(9, 8, 8, '*', 10),
			theadRow:[['STOCK_DT', '일자'], ['COMPANY', '입/출고처'], ['STOCK_FLAG_NAME', '구분'], ['LIST', '내역'], ['REMARKS', '비고']]
		};
		...
		var createTable = function(_searchObj) {
			$.pb.ajaxCallHandler('/adminSec/selectUserStockList.do', _searchObj, function(tableData) {
				...
				$.each(tableData.list, function(idx) {
					...
					_tr.find('td').on('click', function() {
						$('.pb-right-popup-wrap').openLayerPopup({}, function(_thisLayer) {
							...
							$.pb.ajaxCallHandler('/adminSec/selectUserStockStatusList.do', {stockNo:_tr.data('STOCK_NO')}, function(data) {
		...
		$('.register').on('click', function(){
			if(!necessaryChecked($('#dataForm'))) {
				var _url = $(this).val() ? '/adminSec/updateUserStock.do' : '/adminSec/insertUserStock.do';
		...
		$('.delete').on('click', function(){
			if(confirm('삭제하시겠습니까?')){
				var _formData = new FormData($('#dataForm')[0]);
				_formData.append('stockNo', $(this).val());
				$.pb.ajaxUploadForm('/adminSec/deleteUserStock.do', _formData, function(result) {
					$('.top-button-wrap').find('button').prop('disabled', true);
					if(result != 0) {
						createTable(searchObj);
						createSubTable();
...
<div class="contents-title-wrap">
	<div class="title">직원재고</div>
	<div class="title-right-wrap"><button type="button" class="title-button" id="btnRegister">입출고관리</button></div>
</div>

<div class="search-box-wrap">
	<div class="search-left-wrap">
		<div class="search-title">담당자</div>
	</div>
	<div class="search-right-wrap" style="left:60px;">
		<select class="form-select search-user" style="width:160px;"></select>	
	</div>
</div>

<div class="sub-box-wrap">
	<table class="sub-table fixed" style="margin-top:0px;">
		<thead>
	...
	<div class="sub-table-wrap">
		<table class="sub-table list" style="margin-top:0px;">
			<thead>
...
<div class="search-box-wrap">
	...
	<div class="search-right-wrap">
		...
		<input type="text" class="search-text rb" placeholder="담당자명으로 검색"/>
