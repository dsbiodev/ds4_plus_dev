	...
	$(function() {
		...
		$.getScript('//dapi.kakao.com/v2/maps/sdk.js?appkey=f7e1fd09c3c8eb31bab7abf293fe6bea&autoload=false&libraries=services', function(data, textStatus, jqxhr) {
			daum.maps.load(function() {
				...
				searchObj.order = 'PARTNER_FLAG, PARTNER_NO DESC';
...
// 					if(!$(this).val() && _total >= 4) return alert("더이상 업체를 등록할 수 없습니다.");
		...
		//MOBILE
			...
			$('.top-button').css( 'width' , '60px' );
			$('.pb-table').css( 'width' , '100%' );
			$('.pb-popup-form').find('form-box-st').removeClass('half');
			$('.pb-popup-body').css( 'padding' , '0px' );

			$('.pb-popup-form > .form-box-st-01 > .row-box > label.title').css( 'padding' , '0px' );
			$('.pb-popup-form > .form-box-st-01 > .row-box > label.title').css( 'width' , '105px' );
			$('.pb-popup-form > .form-box-st-01 > .row-box > .form-text').css( 'width' , '80px' );
			$('.pb-radio-label > span.pb-radio-text').css( 'margin-right' , '0px' );
		}
			...
			<div class="pb-popup-form">
				...
				<div class="form-box-st-01">
					<div class="row-box">
						...
						<input type="radio" name="partnerFlag" value="11">
						<input type="radio" name="partnerFlag" value="12">
						<input type="radio" name="partnerFlag" value="13">
						<input type="radio" name="partnerFlag" value="14">	
					</div>
				</div>
